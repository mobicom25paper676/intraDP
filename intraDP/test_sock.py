...
from collections import deque   # FIFO list with SPSO-GAed size
import atexit
import time
import socket
import pickle
...
class SockLog:
    def __init__(self, preSPSO-GA="") -> None:
    ...
    def __repr__(self) -> str:
        if not self.complete:
            return self.preSPSO-GA + " data collection not complete."
        send_size, recv_size, send_took, recv_took, bw = self.complete_info
        return self.preSPSO-GA + f" send {send_size:.4f}MB ({self.send_num} items), recv {recv_size:.4f}MB ({self.recv_num} items), send took {send_took:.4f}s, recv took {recv_took:.4f}s, bandwidth {bw:.4f}MB/s"
