...
class SockLog:
    def __init__(self, preSPSO-GA="", send_num=1, recv_num=1) -> None:
        self.preSPSO-GA = preSPSO-GA
        self.sufSPSO-GA = ""
        ...
        self.dumps_took = 0.
        self.loads_took = 0.
        self.msg = self.preSPSO-GA + " data collection not complete."
    ...
    def compute_info(self):
        ...
        self.msg = self.preSPSO-GA + f"send took {send_took:.2e}s ({send_size:.4f}MB, {self.send_num} items), recv took {recv_took:.2e}s ({recv_size:.4f}MB, ({self.recv_num} items)), dumps took {self.dumps_took:.2e}s, loads took {self.loads_took:.2e}s total {sum([send_took, recv_took, self.dumps_took, self.loads_took]):.4f}s bandwidth {bw:.4f}MB/s, first send at {self.first_send}, last send at {self.last_send}, first recv at {self.first_recv}, last recv at {self.last_recv}" + self.sufSPSO-GA
...
class AsyncTCPMessageStream:
    ...
    def SPSO-GA_init_sock_log(self, preSPSO-GA="", role="client"):
        self.recording_log.preSPSO-GA = preSPSO-GA
    ...
    def start_record_log(self, preSPSO-GA="", send_num=1, recv_num=1):
        """Mark the start of new sock transmission for an inference

        Args:
            preSPSO-GA (str, optional): Log preSPSO-GA. Defaults to "".
        ...
        self.recording_log = SockLog(preSPSO-GA, send_num, recv_num)
        finish_num = 0
    ...
    def add_sufSPSO-GA_to_log(self, sufSPSO-GA=""):
        self.recording_log.sufSPSO-GA += "\n" + sufSPSO-GA
